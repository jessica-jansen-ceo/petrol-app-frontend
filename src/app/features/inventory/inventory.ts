...
@Component({
  ...
  template: `
    <app-page-header title="Fuel Inventory" subtitle="Tank stock levels per station. Selling fuel draws stock down automatically.">
...
export class Inventory {
  ...
  readonly stock = this.data.viewStock;
  open = signal(false);
  ...
  modalTitle() { return this.mode() === 'delivery' ? 'Stock Delivery' : this.mode() === 'tank' ? 'Add Tank' : 'Edit Tank'; }
  ...
  openDelivery() {
    ...
    this.fields = [
      { key: 'station', label: 'Station', type: 'select', options: this.data.stationNames(), value: this.data.defaultStationName() },
  ...
  openEdit(s: StockItem) {
    ...
    this.editing.set(s);
    this.fields = [
      { key: 'station', label: 'Station', type: 'text', value: this.data.stationName(s.stationId), readonly: true },
  ...
  save(v: Record<string, string>) {
    if (this.mode() === 'delivery') {
      this.data.adjustStock(this.data.stationIdByName(v['station']), v['fuel'], Number(v['litres']) || 0);
      this.data.log(`Delivery: ${v['litres']}L ${v['fuel']}`);
      this.toast.show('Delivery recorded');
    } else if (this.mode() === 'tank') {
      this.data.add<StockItem>('stock', { stationId: this.data.stationIdByName(v['station']), fuel: v['fuel'], capacity: Number(v['capacity']) || 0, current: Number(v['current']) || 0 });
  ...
  async del(s: StockItem) {
    if (await this.confirm.ask(`Delete the ${s.fuel} tank at ${this.data.stationName(s.stationId)}?`)) {
